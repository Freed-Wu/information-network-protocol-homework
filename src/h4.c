...
void updatestatus(int total, int current, int width) {
	...
	printf("\33[2AProgress: "
	       ...
	       buf, current * 100 / total,
	       (double)current * 512 / pow(1024, lb), unit[lb], speed,
	       (double)(total - current) / 2 / speed);
	free(buf);
}
	...
	if (argc < 2)
		...
		if (argc < 4)
			...
			sendto(sd, buf, cnt, 0, (struct sockaddr*)&serveraddr,
			       sizeof(struct sockaddr_in6));
			do
				cnt = recvfrom(sd, buf, BUFF_LEN, 0,
					       (struct sockaddr*)&peeraddr,
					       &len);
			while (cnt <= 0
			       || memcmp(&serveraddr.sin6_addr,
					 &peeraddr.sin6_addr,
			...
			if (cnt == 2 && buf[0] == 5) {
				...
				for (puts("[\x1B[32m+\x1B[0m] Server "
					  "accepted\n\n"),
				     blkid = 1,
				     pthread_create(&thr, NULL, thr_func,
						    thr_arg),
				     gettimeofday(&tv1, NULL);
				     blkid <= fileinfo.st_blocks; blkid++) {
					cnt = sprintf(buf, "\x03%d ", blkid);
					ret = fread(buf + cnt, sizeof(char),
						    512, fp);
					if (ferror(fp)) {
						fprintf(stderr,
							"[\x1B[31m!\x1B[0m] "
				...
				recv:
					...
					if (acked != blkid || paused)
						goto recv;
					updatestatus(fileinfo.st_blocks, acked,
						     ws.ws_col);
